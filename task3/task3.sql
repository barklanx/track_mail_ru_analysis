...
FROM views v
    JOIN orders o ON v.idhash_order = o.idhash_order
GROUP BY tariff;

-- Теряем больше всего, когда приписывается водитель, и когда приезжает машина
...
FROM orders o
WHERE (order_dttm is not null)
    AND (da_dttm is not null)
GROUP BY idhash_order,
    order_dttm,
    da_dttm
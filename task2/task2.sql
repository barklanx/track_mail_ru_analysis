/*
ЗАДАНИЕ
Выделите для себя определенную тему и проанализируйте ситуацию по ней Например, дайте
полную характеристику матчей, где побеждают силы тьмы (что за участники, каких героев
используют, много ли чего покупают и так далее). Вариантов на самом деле много.
ОФОРМЛЕНИЕ
На ваше усмотрение, вы можете вынести выводы в отдельный документ и расписать, что
интересного на ваш взгляд вы нашли с помощью запросов. Можете просто писать
...
/*
ПОСТАВЛЕННАЯ ЗАДАЧА
Мы хотим сделать так, чтобы чтобы у новых игроков было меньше когнитивных
препятствий для начала игры и более плавный старт в игре. Это поможет нам
привлечь и зантересовать больше начинающих игроков, а также уменьшить количество
игроков которые начинают играть и бросают из-за сложности. Для этого нам
нужно проанализировать и сделать следующее:
    1. Разделить всех неанонимных игроков по классам мастерства (от 1 до 3),
       учитывая разные показатели, чтобы сбалансированно формировать команды;
    2. Посчитать статистики по разным классам и посмотреть каких героев
       предпочитают игроки в разных классах, чтобы рекомендовать героев,
       в зависимости от уровня мастерства игрока.
*/
